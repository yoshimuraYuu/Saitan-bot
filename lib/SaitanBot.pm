...
has 'conf' => {
    is      => 'ro',
    isa     => 'HashRef',
    default => YAML::Syck::LoadFile('config.yml'),
};
...
has 'actions' => {
	...
	Net::Twitter::Lite->new(
	    ...
	    access_token        => $self->conf->{oauth}->{access_token},
	    access_token_secret => $self->conf->{oauth}->{access_token_secret},
	    ssl                 => 1,
...
has 'whoami' => {
	...
	$self->actions->verify_credentials;
    },
};
